...
class DAOTasks {
    ...
    getAllTasks(email, callback) {
       ...
       this.pool.getConnection((err, conexion)=>{
        ...
        const sql ="SELECT t.id, t.text, t.done, g.tag FROM task t LEFT JOIN tag g ON t.id = g.taskId WHERE t.user = ? ORDER BY t.id";

        conexion.query(sql, [email], function(err, tasks){
            if(err){
                callback(err);
            }else{
                let list = [];
    ...
    insertTask(email, task, callback) {
        this.pool.getConnection((err, conexion)=>{
            ...
            const sql1 ="INSERT INTO task (user, text, done) VALUES (?, ?, ?)";
    
            conexion.query(sql1, [email, task.text, task.done], function(err, result){
                if(err){
                    callback(err);
                }else{         
                    const sql2="INSERT INTO tag (taskId, tag) VALUES (?,?)"
                    for(var i = 0; i<task.tags.length; i++){
                    conexion.query(sql2, [result.insertId, task.tags[i]], function(err, result){
                        if(err){
                            callback(err);
                        }
                    });
